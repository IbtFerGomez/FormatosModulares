...
`;
    ...
    const buildTemplateDrivenHtml = (templateInfo) => {
...
<body id="doc-digitalizado-word">
    ...
    const digitalizeDocx = async () => {
        ...
        try {
            ...
            const sourceHtml = extracted.hasMetaBlock ? extracted.contentHtml : (result.value || '');
            ...
            if (templateInfo.isTemplate || forceTemplateMode) {
                generatedHtml = buildTemplateDrivenHtml(templateInfo);
            ...
            generatedCode = extracted.meta.CODIGO_BASE || buildDocumentCode(file.name);
            generatedOutputFileName = extracted.meta.NOMBRE_ARCHIVO_HTML
                ? (extracted.meta.NOMBRE_ARCHIVO_HTML.toLowerCase().endsWith('.html')
                    ? extracted.meta.NOMBRE_ARCHIVO_HTML
                    : `${extracted.meta.NOMBRE_ARCHIVO_HTML}.html`)
                : `documento-digitalizado-${generatedCode || 'word'}.html`;
            generatedHtml = normalizeHtml(sourceHtml, generatedCode, extracted.meta);
            output.value = generatedHtml;
